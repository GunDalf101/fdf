...
/*   Updated: 2022/12/19 06:17:11 by mbennani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int draw_frame(t_fdf *mats)
{
	...
	// {
	// 	for (int or = 0; or < y; or++)
	// 	{
	// 		mlx_pixel_put(mats->mlx, mats->win,	op, or , 0x000000);
	// 	}
	// }
	i = 1;
	while (i < mats->area)
	{
		...
		while (j <= step)
		{
			mlx_pixel_put(mats->mlx, mats->win,	mats->origin->data[0][0] + x0, mats->origin->data[1][0] - y0 , 0xFFFFFF);
			x0 = x0 + xinc;
			y0 = y0 + yinc;
			j++;
		}
